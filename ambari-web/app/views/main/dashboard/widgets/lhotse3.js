...
App.Lhotse3View = App.DashboardWidgetView.extend({
  ...
  transformToSeries: function (jsonData) {
    var seriesArray = [];
    if (jsonData && jsonData.metrics && jsonData.metrics.lhotse_db && jsonData.metrics.lhotse_db.metrics_file && jsonData.metrics.lhotse_db.metrics_file.base_request_count) {
      for ( var name in jsonData.metrics.lhotse_db.metrics_file.base_request_count) {
        var seriesData = jsonData.metrics.lhotse_db.metrics_file.base_request_count[name];
