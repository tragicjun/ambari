...
class TopicToolClient(Script):
    ...
    def start(self, env):
        ...
        topic_tool_client = format("(nohup  python {topic_tool_client}&) &> {new_tube_install_path}/topic_client.log")
    ...
    def status(self, env):
        ...
        process_check_command = "sudo ps aux | grep start_tool_client  | grep -v 'grep' "
    ...
    def stop(self, env):
        import params
        env.set_params(params)
        process_check_command = "sudo ps aux | grep start_tool_client | grep -v 'grep' |  awk '{print $2}' | xargs kill"
        (ret, output) = commands.getstatusoutput(process_check_command)
        Logger.info(process_check_command)
        if ret != 0:
            Logger.error(output)
            Logger.error("can no stop {0}!".format("start_tool_client"))

if __name__ == "__main__":
    TopicToolClient().execute()
