...
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "CREATE DATABASE $GOLDENEYE_DATABASE_GE_NAME"

echo "Create $GOLDENEYE_DATABASE_GE_NAME with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "CREATE DATABASE $GOLDENEYE_DATABASE_MONITOR_NAME"

echo "Create $GOLDENEYE_DATABASE_MONITOR_NAME with result $?"

# Load the lhotse database schema
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS $GOLDENEYE_DATABASE_GE_NAME < $GOLDENEYE_GE_SQL

echo "Load $GOLDENEYE_GE_SQL schema data with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS $GOLDENEYE_DATABASE_MONITOR_NAME < $GOLDENEYE_MONITOR_SQL

echo "Load $GOLDENEYE_MONITOR_SQL schema data with result $?"

# Create the lhotse database user
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "CREATE USER $GOLDENEYE_DB_USER  IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Create user $GOLDENEYE_DB_USER  with result $?"

# Grant the database can be accessed from any machines
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_GE_NAME.* TO '$GOLDENEYE_DB_USER'@'%'"

echo "Grant $GOLDENEYE_DATABASE_GE_NAME privileges for all hosts with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_MONITOR_NAME.* TO '$GOLDENEYE_DB_USER'@'%'"
...
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_GE_NAME.* TO '$GOLDENEYE_DB_USER'@'$MYSQL_DB_HOST' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant $GOLDENEYE_DATABASE_GE_NAME privileges for $MYSQL_DB_HOST with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_MONITOR_NAME.* TO '$GOLDENEYE_DB_USER'@'$MYSQL_DB_HOST' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant $GOLDENEYE_DATABASE_MONITOR_NAME privileges for $MYSQL_DB_HOST with result $?"

#Grant the database to goldeneyey web

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_GE_NAME.* TO '$GOLDENEYE_DB_USER'@'$GOLDENEYE_WEB_HOST' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant $GOLDENEYE_DATABASE_GE_NAME privileges for $MYSQL_DB_HOST with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_MONITOR_NAME.* TO '$GOLDENEYE_DB_USER'@'$GOLDENEYE_WEB_HOST' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant $GOLDENEYE_DATABASE_MONITOR_NAME privileges for $MYSQL_DB_HOST with result $?"

# Grant the database can be accessed from local machine
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_GE_NAME.* TO '$GOLDENEYE_DB_USER'@'localhost' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant privileges for locallhost with result $?"

mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "GRANT ALL PRIVILEGES ON $GOLDENEYE_DATABASE_MONITOR_NAME.* TO '$GOLDENEYE_DB_USER'@'localhost' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant privileges for locallhost with result $?"

# Flush all privileges
mysql -h $MYSQL_DB_HOST -P $MYSQL_DB_PORT -uroot -p$MYSQL_DB_ROOT_PASS -e "FLUSH PRIVILEGES"
