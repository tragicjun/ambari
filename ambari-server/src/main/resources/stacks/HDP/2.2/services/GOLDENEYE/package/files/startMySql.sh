...
echo "Param list: $MYSQL_DB_HOST   $MYSQL_DB_DATA_DIR  $GOLDENEYE_DB_USER    $GOLDENEYE_DB_PASSWORD   $GOLDENEYE_GE_SQL   $GOLDENEYE_MONITOR_SQL"
...
mysql -uroot -e "GRANT ALL PRIVILEGES ON $LHOTSE_DATABASE_NAME.* TO '$GOLDENEYE_DB_USER '@'%'"

echo "Grant privileges for all hosts with result $?"

# Grant the database can be accessed from the db host machine
mysql -uroot -e "GRANT ALL PRIVILEGES ON $LHOTSE_DATABASE_NAME.* TO '$GOLDENEYE_DB_USER '@'$MYSQL_DB_HOST' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"

echo "Grant privileges for $MYSQL_DB_HOST with result $?"

# Grant the database can be accessed from local machine
mysql -uroot -e "GRANT ALL PRIVILEGES ON $LHOTSE_DATABASE_NAME.* TO '$GOLDENEYE_DB_USER '@'localhost' IDENTIFIED BY '$GOLDENEYE_DB_PASSWORD'"
