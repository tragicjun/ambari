...
class Coordinator(Script):
  def install(self, env):
    ...
    print params.coordinator_install
    #su pgxz -c '/usr/local/pgxz/bin/initdb -D /usr/local/pgxz/nodes/coordinator --nodename coordinator'
    val= os.system(params.coordinator_install)
    print val

  def start(self, env):
    import params
    print 'create coordinator config'
    self.configure(env)

    print 'start coordinator'
    print params.coordinator_start
    #su pgxz -c '/usr/local/pgxz/bin/pg_ctl start -D /usr/local/pgxz/nodes/coordinator -Z coordinator'
    val= os.system(params.coordinator_start)
    print val


  def configure(self, env):
    import params
    print 'create the config file by  pgxz().init_coor()';
  ...
  def stop(self, env):
    ...
    print params.coordinator_stop
    val= os.system(params.coordinator_stop)
    print val
     
  def status(self, env):
    import params
    print 'Status of the coordinator';
